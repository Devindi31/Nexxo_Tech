...
if (isset($_SESSION["NexxoTechUser"])) {
    ...
    INNER JOIN `color` ON `ebag`.`color_color_id`=`color`.`color_id` WHERE `user_email`='$email'");
    ...
    if ($ebagNum > 0) {
        ...
        for ($e = 0; $e < $ebagNum; $e++) {
            ...
            $amount = ((int)$ebagData["price"] * (int)$ebagData["ebag_quantity"] + $ebagData["delivery_fee"]);
            ...
            Database::iud("DELETE FROM `ebag` WHERE `ebag_id` = '" . $ebagData["ebag_id"] . "' AND `user_email`='$email'");
        }
