...
if (isset($_SESSION["service"])) {
    ...
    $customerName = $_GET["search"];

    $messageResult = Database::search("SELECT `message`.`customer`,`user`.`fname`,`user`.`lname`,MAX(`message`.`date_time`) AS latest_date_time FROM `message` 
    INNER JOIN `user` ON `message`.`customer`=`user`.`email` WHERE `service` = '$service' AND (`fname` LIKE '%$customerName%' OR `lname` LIKE '%$customerName%') GROUP BY `message`.`customer` ORDER BY `latest_date_time` DESC");
