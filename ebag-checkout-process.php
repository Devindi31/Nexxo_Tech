...
if (isset($_SESSION["NexxoTechUser"])) {
    ...
    if ($addressNum > 0) {
        ...
        INNER JOIN `product` ON `ebag`.`product_product_id`=`product`.`product_id` WHERE `user_email`='$email'");
        ...
        if ($ebagNum > 0) {
            ...
            $amount = $totalAmount = (int)$subTotal + (int)$ebagData["delivery_fee"];
